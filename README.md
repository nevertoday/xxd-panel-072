<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 072 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 072 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
锁定轮廓、结构、姿态与叙事关系 → 删除写实细节 → 几何化为平面色块和纤细线条 → 建立区域差异化的磨砂、柔焦与透叠层次 → 保留原图色相身份并提亮净化 → 在浅色大留白中居中平衡 → 加入克制的现代编辑文字
```

## 成品中最容易识别的风格特点

- 主体只保留最值得记住的身份、姿态、轮廓与叙事关系，不完整复刻复杂场景。
- 主体以简洁几何、平面色块、纤细线条和大留白高度概括，但核心识别特征不丢失。
- 磨砂、虚化和透叠具有清楚的区域差异与层级，不做无差别的整块平均模糊。
- 居中主体、正负形、尺度关系和浅色大留白共同形成安静而高级的悬浮感。
- 色彩保留原图色相身份，再提亮、提纯、去灰、净化与柔化，不采用外层固定色板。
- 文字少量、现代、精致，像国际设计工作室、建筑海报或艺术展览的编辑性排版。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-072-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–072） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
