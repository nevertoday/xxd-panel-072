...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 072이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 072**는 가장 식별하기 쉬운 윤곽, 구조, 자세, 관계를 간결한 기하 형태, 평면 색면, 가는 선으로 요약하고 영역마다 강도가 다른 반투명 프로스트 창을 만듭니다. 선명한 부분, 부드럽게 흐려지는 부분, 투명하게 겹치는 부분이 공존하며 피사체는 한눈에 알아볼 수 있는 채 조용한 밝은 공간에 떠 있습니다. 절제된 현대적 문자가 건축 포스터와 전시 그래픽 같은 질서를 더합니다.

### 이런 경우에 적합합니다

- 필터처럼 보이지 않는 반투명 유리를 원하며, 화면 전체의 평균 흐림이 아니라 영역과 층에 따라 투명도를 설계하고 싶을 때
- 추상화해도 한눈에 알아보도록 핵심 윤곽, 구조, 자세와 서사 관계가 장식에 묻히지 않기를 원할 때
- 중앙 균형, 포지티브·네거티브 형태, 여백, 구성선과 절제된 글자로 현대적 시각 시스템을 만들고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 072로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 반투명 프로스트 창 · 영역별 소프트 포커스 · 미니멀 기하 · 식별 가능한 윤곽 · 선명하고 치유적인 색 · 현대 편집 문자
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 선명한 식별성과 흐릿한 공기감을 함께 지닌 고급스럽고 현대적인 프로스트 유리 추상 포스터 |
| **대표 특징** | 넓은 여백에 떠 있는 기하화된 피사체와 영역마다 다른 소프트 포커스, 투명 겹침, 선명한 경계 |
| **입력 존중 방식** | 핵심 윤곽, 구조, 자세, 관계를 유지하고 원본 색상 정체성을 보존한 뒤 밝고 깨끗하고 부드럽게 조정함 |
...
```text
윤곽·구조·자세·서사 관계 고정 → 사실적 세부 삭제 → 평면 기하와 가는 선으로 요약 → 영역별 프로스트·소프트 포커스·투명 겹침 구축 → 원본 색상 정체성을 보존하며 밝고 깨끗하게 조정 → 밝은 넓은 여백에 중앙 배치 → 절제된 현대 편집 문자 추가
```

## 완성작의 특징

- 간결한 기하, 평면 색면, 가는 선, 넓은 여백으로 피사체를 요약하면서 식별 특징을 유지합니다.
- 흐림, 초점 이탈, 투명 겹침에 분명한 영역 차이와 위계를 주고 균일한 효과를 피합니다.
- 중앙 배치, 양과 음의 형태, 크기, 밝은 여백으로 조용한 부유감을 만듭니다.
- 원본 색상 정체성을 보존한 뒤 밝기와 순도를 높이고 회색기를 제거하며 깨끗하고 부드럽게 만듭니다.
- 문자는 적고 현대적이며 정교하게 하여 건축 포스터와 전시 편집 시스템에 가깝게 구성합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-072-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 072의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-072 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-072 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-072 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-072 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-072 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-072 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 072 적용, 공통 설정은 한 번만 확정
/xxd-panel-072 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-072`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-072-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-072-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

72개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 072까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–072) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
