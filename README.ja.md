...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 072 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
**Panel 072** は、最も識別しやすい輪郭、構造、姿勢、関係を簡潔な幾何学、平坦な色面、細い線へ要約し、領域ごとに強さの異なる半透明のすりガラス窓を構築します。明瞭な場所、柔らかくぼける場所、透過して重なる場所が共存し、被写体は一目で分かるまま静かな淡色空間に浮かびます。抑制された現代的な文字が、建築ポスターや展覧会グラフィックの秩序を与えます。

### 向いている用途

- フィルター感のない磨りガラスを求め、画面全体を均等にぼかすのではなく領域と層ごとに透明度を設計したい場合
- 抽象化しても一目で認識できるよう、核となる輪郭、構造、姿勢、物語関係を装飾に埋もれさせたくない場合
- 中央の均衡、正負形、余白、構図線、抑制された文字で現代的なビジュアルシステムをつくりたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 072 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 半透明すりガラス窓 · 領域差のあるソフトフォーカス · ミニマル幾何学 · 識別できる輪郭 · 鮮明で癒やしのある色 · 現代的編集文字
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 明瞭な識別性と霞んだ空気感を両立する、上質で現代的なすりガラス抽象ポスター |
| **識別しやすい特徴** | 大きな余白に浮く幾何学化された主体と、領域ごとに異なる柔焦、透過、明瞭な境界 |
| **入力の尊重** | 中核の輪郭、構造、姿勢、関係を保ち、元写真の色相を維持してから明るく清潔に柔らかくする |
...
```text
輪郭、構造、姿勢、物語関係を固定 → 写実細部を削除 → 平坦な幾何学と細線へ要約 → 領域差のあるすりガラス、柔焦、透過層を構築 → 元写真の色相を保ちながら明るく清潔にする → 淡色の大きな余白で中央配置 → 抑制された現代編集文字を加える
```

## 完成作の特徴

- 簡潔な幾何学、平坦な色面、細線、大きな余白で主体を要約しながら、識別特徴を維持する。
- ぼけ、焦点外、透過の重なりに明確な領域差と階層を与え、一様な効果にしない。
- 中央配置、正負形、尺度、淡色の余白で静かな浮遊感をつくる。
- 元写真の色相を保ち、明るさと純度を上げ、灰みを除き、清潔で柔らかくする。
- 文字は少量、現代的、精緻にし、建築ポスターや展覧会の編集組版に近づける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-072-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 072 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-072 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-072 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-072 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-072 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 072 を適用し、共通設定は一度だけ解決
/xxd-panel-072 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-072`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-072-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-072-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

72 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 072 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–072） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
